...
if( in_array($pagenow, array('post-new.php', 'post.php'))) {
	...
	wp_enqueue_style('gallerybox-admin', plugins_url('/assets/css/gallerybox-admin.css', __FILE__), array(), '1.7.4', 'all' );
...
add_action( 'plugins_loaded', 'gbox_image_size' );
    ...
    function gbox_adminpro_link($links) {
        $newlink = sprintf("<a target='_blank' href='%s'><span style='color:red;font-weight:bold'>%s</span></a>",esc_url('https://wpthemespace.com/product/gallery-box-pro/'),__('Go Pro','optionsdemo'));
        $links[] = $newlink;
        return $links;
    }
